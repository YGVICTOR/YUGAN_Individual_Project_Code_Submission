...
"""
    This script loads the final GRU models selected by grid search methods, prints their architecture
...
# ticker  = 'AAPL'
# predicted the next day's lose price.
pre_days = 1

number_of_training = 2300


filePath = './model/GRU/final_model'
...
for part_of_dir in all_result_list:
    ...
    print(model_dir)
    ticker = model_dir.split(sep='./model/GRU/final_model/')[1].split(sep='_')[0]
    ...
    # using memory_his_days for predicted
    ...
    scalar_X,X_train,X_test,Y_train,Y_test = stock_price_GRU_pre_processing(pd.read_csv('./Feature_Engineering/result/'+ticker+'.csv').drop('Unnamed: 0', axis=1),memory_his_days)
    # saving architecture
    ...
    if flag == 'with':
        title = 'GRU prediction of ' + ticker + ' with google trend index'
    else:
        title = 'GRU prediction of ' + ticker + ' without google trend index'
    plt.title(title,fontsize=font_size_big)
    plt.grid()
    # if flag =='without':
    #     plt.savefig(f'GRU_{ticker}_without_trend_line.png')
    # else:
    #     plt.savefig(f'GRU_{ticker}_with_trend_line.png')
    plt.show()

    if flag =='without':
        print(f'GRU_{ticker}_without_trend')
    else:
        print(f'GRU_{ticker}_with_trend')
...
#         path = f'{ticker}_GRU_{flag}_trend_architecture.png'
#         # src.write_png(path)
